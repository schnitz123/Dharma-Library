---
{"dg-publish":true,"permalink":"/tibetan-grammar/grammar-particles/"}
---

ཚིག་ཕྲད་ཀྱི་གོ་དོན་ནི། ཚོགས་གསུམ་གང་རུང་གི་ཐོག་མཐའ་བར་གསུམ་ཅི་རིགས་ཀྱི་གནས་སུ་སྒྲ་དེ་སྦྱར་ན་དོན་ཞིག་སྟོན་ལ།
དེ་ལྟར་མ་སྦྱར་བའི་སྒྲ་རྐྱང་པ་དེས་དོན་གསལ་པོ་གང་ཡང་མི་སྟོན་པའོ།
**Understood meaning:** A term that indicates [some] meaning when affixed to the beginning, middle or end of any of the three collections, but does not indicate any clear meaning by itself when not affixed in such a way.

ཕྲད། ཚིག་ཕྲད། ཕྲད་ཀྱི་སྒྲ། are mutually inclusive.

**Divisions:**
- [[Tibetan Grammar/The eight cases\|The eight cases]] རྣམ་དབྱེ་བརྒྱད།
- [[Tibetan Grammar/Fourteen grammar particles\|Fourteen grammar particles]] ཚིག་ཕྲད་བཅུ་བཞི།
- Other:
	- [[Tibetan Grammar/Reason Indicators\|Reason Indicators]]
	- [[Tibetan Grammar/Speech Marks\|Speech Marks]]
	- [[Tibetan Grammar/Indefinite Articles and Imperative Markers\|Indefinite Articles and Imperative Markers]]
	- [[Tibetan Grammar/Focusing adverbs\|Focusing adverbs]]

Three possibilities between a grammar particle and a case:
- If it is a case, then it is necessarily a grammar particle
- If it is a grammar particle, then it is not necessarily a case; e.g. ནི།

Taking ལ། as an example:
- Used as the second, fourth, and seventh cases, it is both
- In non-case usages, it is a grammar particle but not a case; e.g. ཚིག་ཕྲད་ཡིན་ལ་རྣམ་དབྱེ་མ་ཡིན་པའི་མུ།

---
Up a level: [[Tibetan Grammar/Tibetan Grammar\|Tibetan Grammar]]